-- BrainBattle Database Setup Script (Fixed Version)
...
-- First, check if users exist before inserting
DO $$
BEGIN
    -- Insert admin user if not exists
    IF NOT EXISTS (SELECT 1 FROM users WHERE email = 'admin@brainbattle.com') THEN
        INSERT INTO users (username, email, password_hash) 
        VALUES ('admin', 'admin@brainbattle.com', '$2a$10$example_hash_here');
    END IF;
    
    -- Insert test user if not exists
    IF NOT EXISTS (SELECT 1 FROM users WHERE email = 'test@brainbattle.com') THEN
        INSERT INTO users (username, email, password_hash) 
        VALUES ('testuser', 'test@brainbattle.com', '$2a$10$example_hash_here');
    END IF;
END $$;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM player_stats WHERE user_id = (SELECT id FROM users WHERE username = 'admin')) THEN
        INSERT INTO player_stats (user_id, level, xp, total_wins, total_losses)
        SELECT id, 1, 0, 0, 0 FROM users WHERE username = 'admin';
    END IF;
    
    -- Create stats for test user
    IF NOT EXISTS (SELECT 1 FROM player_stats WHERE user_id = (SELECT id FROM users WHERE username = 'testuser')) THEN
        INSERT INTO player_stats (user_id, level, xp, total_wins, total_losses)
        SELECT id, 1, 0, 0, 0 FROM users WHERE username = 'testuser';
    END IF;
END $$;
